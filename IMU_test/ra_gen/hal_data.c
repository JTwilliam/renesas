...
{ .offset = 1, .src_buffer_size = 1,
  ...
  .channel = 0, .p_callback = NULL, .p_context = NULL, .activation_source = ELC_EVENT_ADC0_SCAN_END, };
        ...
        { .output_enabled = false, .stop_level = GPT_PIN_LEVEL_LOW },
          ...
          .capture_b_source = (gpt_source_t) (
                  GPT_SOURCE_GTIOCB_FALLING_WHILE_GTIOCA_LOW | GPT_SOURCE_GTIOCB_FALLING_WHILE_GTIOCA_HIGH
                          | GPT_SOURCE_NONE),
          .capture_a_ipl = (4), .capture_b_ipl = (4), .compare_match_c_ipl = (BSP_IRQ_DISABLED), .compare_match_d_ipl =
                  (BSP_IRQ_DISABLED),
...
/* Actual period: 0.00065536 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x10000,
  .duty_cycle_counts = 0x8000, .source_div = (timer_source_div_t) 0, .channel = 2, .p_callback = gpt2_callback,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
{ .channel = 2, .rate = I2C_MASTER_RATE_STANDARD, .slave = 0x78, .addr_mode = I2C_MASTER_ADDR_MODE_7BIT,
